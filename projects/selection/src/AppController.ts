...
import Loader, { registerLoaderPlugin } from "bg2e-js/ts/db/Loader.js";
...
export default class MyAppController extends SceneAppController {
  ...
  private async createSphereNode({
    ...
    const plist = createSphere(0.2);
    sphereNode.addComponent(new Drawable())
    sphereNode.drawable?.addPolyList(plist, await Material.Deserialize({
  ...
  async loadScene() {
    ...
    const cubeDrawable = await loader.loadDrawable("./simple_cube.obj");
    const cubeNode = new Node("Cube");
    cubeNode.addComponent(new Transform(
      Mat4.MakeIdentity()
        .translate(0, 0.5, 0)
        .scale(0.2, 0.2, 0.2)
    ));
    cubeNode.addComponent(cubeDrawable);
